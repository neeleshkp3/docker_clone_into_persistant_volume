...
if [ "$3" = "" ]
then
        echo "Please provide a destination mount point directory in which volume will be created"
        exit
fi
...
if [ -d "$DIR" ]; then
  ...
  echo "Creating Persistant Volume named $2 in $3/"
else
...
docker volume create -d local-persist -o mountpoint=$3/$2 --name $2
